	...
	namespace files
	{
			...
			std::size_t hash_offset, std::size_t hash_offset_size)
		{
			...
			if (hash_offset != 0 && hash_offset_size != 0)
			{
				...
				get_uncertificated_size(file, &real_size);
			...
			HANDLE object = CreateFileMapping(file, NULL, 2, 0, 0, NULL);
			...
			unsigned char* base_address = reinterpret_cast<unsigned char*>(MapViewOfFile(object, 4, 0, 0, file_size));
		...
		bool read_header(unsigned char* output, file_type filetype, std::size_t length, std::size_t start)
		{

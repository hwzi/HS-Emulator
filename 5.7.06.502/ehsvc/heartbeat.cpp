...
namespace ahn
{
	...
	void heartbeat::type_handler_0001(buffer::request* request, buffer::response* response)
	{
		...
		response->add<unsigned int>(0x00000000); /* if (sub_10030BE0(v7, dword_1014E844, buffer)) */
		response->add<unsigned int>(0x00000000);

		response->add<unsigned int>(static_cast<unsigned char>(0x01));
		response->add<unsigned int>(0x00000000 /* GetValue(byte_10154EB4) == 0 */);
	}
	...
	{
		unsigned short flags[] = { 0x0020, 0x0001, 0x0010, 0x0002, 0x0004, 0x0008, 0x0040, 0x0080, 0x0100, 0x0200, 0x0400, 0x0800, 0x1000 };
		...
		std::function<void(buffer::request*, buffer::response*)> flag_handlers[] =
		{
			...
			std::bind(&heartbeat::flag_handler_0200, this, request, response), //not coded in yet
	...
	{
		/* Sent with flag 0x0009, simply a heartbet-error indicator */
	}
